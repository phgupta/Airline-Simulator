...
void City::calcDistance(const City *city2, int *ppl, int *dist, int *opt) const
{  
  *ppl = (double) population * city2->population / 2500000000U;
  *dist = acos(
  ...
  if (*opt != 5)
  {
    cout << *ppl << " passengers fly the " << *dist << " miles from\n";
    cout << name << ", " << state << " to "
